...
| 応募スペシャリスト | **確認済み** |
| 需要探索・収益化 | **確認済み**（X自動投稿例外は **2026-08-30 停止**） |
| 競輪予想トライアル | **Cursorへ引き継ぎ済み**（Grok説明文は不要との回答） |
| 案件スカウト | **確認済み** |
| 自動化オペレーター | **確認済み** |
| サイドバー上のBot名・グループ | スクショで確認済み |
| Threadsチーフ / 受信箱仕分け / 運用マネージャー など | **未確認** |
| `profile.json` 実体 | **未確認** |

次に中身確認する優先順：運用マネージャー → Threadsチーフ → 受信箱仕分け。
...
RECEIVES FROM
副業チーフ、Threadsチーフ、応募スペシャリスト、成果物下書き。統括（最終Gate/P0/INTERNALのみ）。成長（市場事実）。スカウト（怪しい条件）。
...
| 備考 | `note下書き` と同系統の親配下に新規作成。Grok側プラグインが同じGoogleアカウントに繋がっていること |

Grokの成果物下書きプロフィールに、上表をそのまま貼る。
...
RECEIVES FROM
副業チーフ（Yesチケット必須）。統括INTERNAL。Threadsチーフ（定型の下準備のみ）。
...
- 競輪実務は Cursor 側
- 成果物下書きの Drive セットアップは記入済み。Grokプロフィールへ反映が必要
- 需要探索のX自動投稿は停止。Grokプロフィールから例外文の削除が必要
- 次の未確認優先：運用マネージャー → Threadsチーフ → 受信箱仕分け
