...
- [ ] CLI限定委任：Cursor往復と両CLIのアクセス制限、新しい会話での適用は未完了・未確認。自動委任は双方無効。mainは未反映（`docs/CLI_DELEGATION_SETUP.md`）

---

## ✅ 完了

- [x] CLI限定委任のスキル・共通入口・Windows同期方法を作成し、PCへ先行配置。Geminiの文章往復とCodexの共通スキル検出を確認（自動委任の有効化とは別）
